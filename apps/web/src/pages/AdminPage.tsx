...
function AuditDetails({ audit }: { audit: AuditLog }) {
      ...
      <div className="space-y-1.5">
        {changes.map((change, index) => (
          <div key={`${change.field}-${index}`} className="rounded-lg bg-slate-50 px-2 py-1.5 leading-4">
            ...
            <span className="px-1 text-slate-400" aria-hidden>â†’</span>
  ...
  if (audit.metadata?.nextRole) return <>{audit.metadata.previousRole} â†’ {audit.metadata.nextRole}</>;
  return <>{audit.metadata?.resourceType ?? 'â€”'}</>;
}
          ...
          <div className="mt-3 grid max-h-[520px] gap-3 overflow-y-auto pr-1 scrollbar-soft sm:hidden">
            {audits.length ? audits.map((audit) => (
              <article key={audit._id} className="rounded-2xl border border-slate-900/10 bg-white/75 p-4 shadow-sm">
                ...
                <p className="mt-1 break-all text-xs font-semibold text-slate-500">{audit.actorUserId?.name ?? 'System'} Â· {audit.actorUserId?.email ?? 'Automated'}</p>
